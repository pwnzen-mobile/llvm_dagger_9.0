...
DCRegisterSema::DCRegisterSema(const MCRegisterInfo &MRI,
  ...
  for (auto RCI = MRI.regclass_begin(), RCE = MRI.regclass_end();
    ...
    unsigned SizeInBits = RCI->getNumRegs() * 8;  //新版本接口进行了修改
    for (auto Reg : *RCI) {
...
void DCRegisterSema::FinalizeFunction(BasicBlock *ExitBB) {
  ...
  saveAllLocalRegs(ExitBB, ExitBB->getTerminator()->getIterator()); //获取instruction对应的iterator
...
void DCRegisterSema::createLocalValueForReg(unsigned RegNo) {
  ...
  if (LargestSuper != RegNo) {
    ...
    Builder->SetInsertPoint(EntryBB, EntryBB->getTerminator()->getIterator());
...
Function *DCRegisterSema::getOrCreateRegSetDiffFunction(bool Definition) {
 ...
 // Function *RSDiffFn = cast<Function>(TheModule->getOrInsertFunction(
  //    "__llvm_dc_print_regset_diff",
   ...
   Function *RSDiffFn = dyn_cast<Function>(TheModule->getOrInsertFunction(
      "__llvm_dc_print_regset_diff",
      FunctionType::get(Builder->getVoidTy(), RSDiffArgTys, false)).getCallee());
