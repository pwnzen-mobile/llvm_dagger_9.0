...
StringRef MCMachObjectSymbolizer::findExternalFunctionAt(uint64_t Addr) {
  ...
  if (auto ec = SymNameOrErr.takeError())
    report_fatal_error(std::move(ec));
...
uint64_t MCObjectSymbolizer::getEntrypoint() {
  for (const SymbolRef &Symbol : Obj.symbols()) {
    ...
    if (auto ec = NameOrErr.takeError())
      report_fatal_error(std::move(ec));
    StringRef Name = *NameOrErr;

    if (Name == "main" || Name == "_main") {
      Expected<uint64_t> EntrypointOrErr = Symbol.getAddress();
      if (auto ec = EntrypointOrErr.takeError())
        report_fatal_error(std::move(ec));
...
void MCObjectSymbolizer::buildAddrToFunctionSymbolMap() {
  ...
  for (const SymbolRef &Symbol : Obj.symbols()) {
    ...
    if (auto ec = SymAddrOrErr.takeError())
      report_fatal_error(std::move(ec));
    ...
    if (SymName.empty() || SymType.get() != SymbolRef::ST_Function)
      continue;
